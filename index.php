  ...
  public function getDailyActivity($file)
  {
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
      ...
      for ($col = 'A'; $col <= $highestColumn; $col++) {
        ...
        $getColumnIndex = $worksheet->getCell($col . "1")->getValue();
        $cellValue = $worksheet->getCell($col . $row)->getValue();

        if ($getColumnIndex == "MessageStream") {
          $tmpRow["A"] = $cellValue;
        } else if ($getColumnIndex == "To/0/Email") {
          $tmpRow["B"] = $cellValue;
        } else if ($getColumnIndex == "Recipients/0") {
          $tmpRow["C"] = $cellValue;
        } else if ($getColumnIndex == "Recipients/1" && !empty($cellValue)) {
          $tmpRow["C"] = $cellValue;
        } else if ($getColumnIndex == "ReceivedAt") {
          $tmpRow["D"] = $cellValue;
        } else if ($getColumnIndex == "From") {
          $tmpRow["E"] = $cellValue;
        } else if ($getColumnIndex == "Subject") {
          $tmpRow["F"] = $cellValue;
        } else if ($getColumnIndex == "Status") {
          $tmpRow["G"] = $cellValue;
        }
      }

      if ($tmpRow["B"] == "grace.mendoza@robinsonsland.com") {
        $tmpRow["C"] = "lea.tan@robinsonsland.com";
      }
